...
def test_mime_type_in_xattr_for_html(test_root_path):
...
def test_mime_type_in_xattr_for_text(test_root_path):
    """The mime type is available in the extended attributes of the file so check the magic mime type detection is working reasonably"""
...
def test_mime_type_in_xattr_for_empty(test_root_path):
    """The mime type is available in the extended attributes of the file so check the magic mime type detection is working reasonably"""

    # Purposefully avoid using a file extension so it's not tempting to use it for the content type

    with tempfile.NamedTemporaryFile(dir=test_root_path, prefix=sys._getframe().f_code.co_name, mode="w+t", encoding="utf-8") as temp_file:
        temp_file.flush()
        # ; charset=utf-8
...
def test_mime_type_in_xattr_for_png_from_file_extension(test_root_path):
    """The mime type is available in the extended attributes of the file so check the mimetype detection is working reasonably"""

    with tempfile.NamedTemporaryFile(dir=test_root_path, prefix=sys._getframe().f_code.co_name, mode="w+b", suffix=".png") as temp_file:
        temp_file.flush()
